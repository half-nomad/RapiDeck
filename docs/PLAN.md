...
#### 체크리스트
- [ ] **Gemini API 연동**
  - [ ] GeminiClient 서비스 클래스 구현
  - [ ] API 키 로컬 변수 저장 (간단한 폼)
  - [ ] 시스템 프롬프트 로드 (docs/SYSTEM_PROMPT.md)
  - [ ] 에러 핸들링 구현

- [ ] **문서 파싱**
  - [ ] DocumentParser 서비스 구현
  - [ ] 텍스트 입력 받기
  - [ ] 30페이지 제한 로직
  - [ ] 초과 시 자동 잘라내기 + 경고

- [ ] **HTML 슬라이드 생성**
  - [ ] SlideGenerator 서비스 구현
  - [ ] Gemini 응답 → HTML 저장
  - [ ] 미리보기 페이지 (간단한 iframe)
...
- [ ] **컨텍스트 관리**
  - [ ] 세션 기반 대화 이력 저장 (메모리, DB 없이)
  - [ ] Gemini 재호출 시 이전 메시지 포함
